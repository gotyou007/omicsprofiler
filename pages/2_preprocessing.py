...
try:
    ...
    padj_mtds = {"DeSeq2": "DeSeq", "Limma": "limma"}
    ...
    if exprdict and metadatadict:
        ...
        padj_cutoff = prep_exp.select_slider("padj cutoff", value=0.05, options=[0.001, 0.005, 0.01, 0.05])
        ...
        deseq2_counts = dds.run_norm()
        #res_df = dds.run_deseq2() 
        ...
        st.write("DeSeq2_res", res_df)
        #limma 
        limmaM = limmaProcess(counts_df.T, metadata, reads_cutoff, 
                              padj_cutoff, st.session_state['contrast'])
        res_df_limma = limmaM.run_limma()
        st.write("limma", res_df_limma)
    else:
        #preprocess data
        pass
except KeyError:
